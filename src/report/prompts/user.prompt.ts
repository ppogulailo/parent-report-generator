...
function classifySeverityEn(
  ...
  if (tier === 'SERIOUS') {
    return {
      level: 'SERIOUS',
      guidance:
        'Inputs show multiple strong concerns or elevated safety signals. Grounded urgency — calm, direction-giving, never alarmist. Name the emotional weight directly only where the inputs actually show it. ASAP-endorsed professional referral may appear in the FIRST 72 HOURS PLAN. Reference external emergency resources only if acute-risk signals are explicit (suspected fentanyl/heroin, safety crisis, suicidality). Primary ASAP Discussion Group is "Monitoring and Intervention" (NOT "Parent Support Forum"). HARD RULE: every single time you write "Monitoring and Intervention" anywhere in the plan, name "Sustaining Recovery" in the same bullet/paragraph as the place to post questions if the parent decides to pursue professional help — the two names always travel together, no exceptions. Every mention of professional support must point the parent to the Admin Space — that is where ASAP-endorsed treatment providers and therapists are listed.',
    };
  }
  if (tier === 'MILD') {
    return {
