...
function checkCitations(
  ...
  for (const m of matches) {
    ...
    const matchesAny = [...valid].some(
      (v) =>
        normalised === v ||
        normalised.startsWith(v) ||
        v.startsWith(normalised),
    );
    if (!matchesAny) {
...
export function validateReportResources(
  ...
  warnings.push(
    ...checkCitations(fullText, ARTICLE_CITE, ARTICLE_TITLES, 'article'),
  );
  warnings.push(
    ...checkCitations(fullText, WORKSHOP_CITE, WORKSHOP_TITLES, 'workshop'),
  );
  warnings.push(
    ...checkCitations(fullText, GROUP_CITE, GROUP_TITLES, 'discussion-group'),
  );
