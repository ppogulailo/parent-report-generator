...
// Raised 500 → 1500 so Spanish emergency notes aren't cut off (founder testing).
...
export default function PageClient({ language }: Props) {
  ...
  return (
    <main>
      ...
      {/* The plan is generated by the backend directly in the language the
          user selected (EN or ES), so it must display in that language from
          the first streamed token through completion — it should never be
          re-translated by the browser. We therefore keep translate="no" on
          this region PERMANENTLY. This also (a) matches the founder's
          requirement that generation always match the selected language start
          to finish, and (b) keeps Chrome/Google Translate out of the subtree
          entirely — which previously raced React's streaming mutations,
          dropped sentence tails, and caused the insertBefore crash. Earlier we
          released the gate (translate="yes") once streaming finished; that
          caused the completed plan to visibly flip languages, so it is gone. */}
      {hasAnyReport && (
        <div
          className="results"
          translate="no"
