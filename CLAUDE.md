...
- **Domains overlap.** q18 and q22 each count toward two, so `questionIds` lives on the domain rather than a `domainId` on the question. q04 was domainless in the old path; Dave approved adding it to Immediate Safety & Urgency (2026-08-25), applied on the V1 path only via `APPROVED_DOMAIN_AMENDMENTS` in the generator — the old path's `DOMAIN_MAP` is deliberately untouched, and the parity test amends its own copy to match.
